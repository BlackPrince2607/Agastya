...
import { LEGAL_URLS, type LegalDocId } from '@/constants/legal';
...
export function LegalDocumentScreen({ docId }: { docId: LegalDocId }) {
  const document = LEGAL_DOCUMENTS[docId];
  const webUrl = LEGAL_URLS[docId];
  ...
  return (
    <CosmicScreen>
      ...
      <ScrollView
        ...
        {docId !== 'privacy' ? (
          <Pressable onPress={() => openLegalDoc('privacy')} className="mt-4 py-2">
        ...
        {docId !== 'terms' ? (
          <Pressable onPress={() => openLegalDoc('terms')} className="mt-1 py-2">
            <Text style={{ color: colors.secondary }} className="font-body text-[13px]">
              View Terms of Use
            </Text>
          </Pressable>
